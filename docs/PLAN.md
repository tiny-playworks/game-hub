...
- 阶段 1/2：已落地并持续小迭代
- 阶段 3：**进行中（最小闭环已落地）** — 档案 `audioVolumes`（BGM / SFX / 语音）与对局 `useRiichiSounds`（SFX×wav/beep、语音×TTS）已接；`discard.wav` / `draw.wav` 优先接入，缺失时 beep 兜底；**大厅/对局 BGM 与全局 UI 点击音**仍待扩展（见 [sound-assets.md](sound-assets.md)、[player-meta-backlog.md](player-meta-backlog.md) B-A1）

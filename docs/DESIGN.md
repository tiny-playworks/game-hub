...
| **QuickAccessPanel** | 签到、成就、档案、语言；有 `compact` | 首页 header、日麻 header |
...
**视图状态**：`rules` → `game`（含 `guide` 侧栏）

**桌面布局概念**：

```
┌ Header：返回 | 局况 | 宝牌 | 主题 | 菜单 ─┐
├──────────────────────────────────────────┤
│     [对家]  点数  立直棒  手牌背              │
│  [上家]          牌桌中央            [下家] │
│              河牌 / 副露                     │
├──────────────────────────────────────────┤
│  己方手牌（可点击出牌）  |  操作栏：立直吃碰杠胡 │
│  状态条：听牌提示 / 振听 / 计时                │
└──────────────────────────────────────────┘
```

**移动端**：`RiichiMobileStage` 单列；牌放大；操作区底部固定；虚拟安全区。
...
- **安全区**：日麻 mobile 需 `env(safe-area-inset-*)`
- **横屏**：小游戏、日麻可选支持；优先竖屏单手

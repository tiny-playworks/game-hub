...
| 已上线游戏 | **18 款**：9 小游戏 + 4 棋类（五子棋、中国象棋、国际象棋、围棋）+ 3 麻将 + 2 扑克 |
| 占位游戏 | **0 款**（斗地主、升级已上线） |
...
- **麻将（3）**：四川麻将、中国通用麻将、日本立直麻将（和了算分由 riichi-rs-bundlers 提供）  
- **扑克（2）**：斗地主（简化版）、升级（入门）  
...
├── pages/           # Home, Category, Achievements, Game*（18 个游戏页）
...
- **规模**：7 个测试文件，**60 个用例**，全部通过。  
- **覆盖**：数据与工具、首页与分类、成就页、18 个游戏页冒烟、五子棋胜负、麻将规则（国标/川麻/日麻）。  
...
| 维度 | 状态 | 说明 |
|------|------|------|
| 核心玩法 | ✅ 高 | 18 款可玩，规则闭环（含三种麻将、四棋、两扑克） |
...
**需求排序以 [PLAN.md](PLAN.md) 为准**；此处为详表与功能清单。
...
### 5.2 后续优先级（需求排序见 [PLAN.md](PLAN.md)）

| 优先级 | 项 | 说明 |
|--------|----|------|
| **P2** | 麻将变种 / 更多语音与动画 | 按需：其他游戏音效与动画、更多麻将规则 |
